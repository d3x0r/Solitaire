...
export class card_drag_control extends Events {
		...
		case "start":
			...
			for( let touch of e.changedTouches ) {
				...
				if( old ) {
					...
					this.touches.push( { ID: touch.identifier, x: touch.clientX
					                   , y: touch.clientY
					                   , new: true
					} )
				}
	...
	addTurn( card, start, end ) {
		...
		this.turning.unshift(
		     { card, start: this.startOfs + performance.now() / 1000 + start
		     , end: this.startOfs + performance.now() / 1000 + end
		     } );
	...
	addMove( from, to, start, end, card ) {
		...
		card.flags.bFloating = true; // table draw code shouldn't draw this until
	...
	touchUpdate() {
		...
		if( this.touches.length == 1 ) {
			...
			console.log( "Too many touches?", this.touches.length );
	}
	...
	static mouse( self, x, y, b ) {
		...
		if(0)
			msg.textContent
			     = ( "Drag event:" + x.toFixed(2) + "," + y.toFixed(2) + "," + b + " " + self.ofsx.toFixed(2) + ","
			       + self.ofsy.toFixed(2) );
		//if( !( self._b & 1 ) && ( b & 1 ) ) {
			//self.setDrag( x, y );
		...
		if( ( self._b & 1 ) && !( b & 1 ) ) {
			...
			for( let stack of self.stacks ) {
				...
				if( card_stack_control.CanMoveCards( self.stack, stack ) ) {
					possibles.push( {
					   near: { x: stack.left + stack.image_width / 2 - x
					         , y: stack.top + stack.image_height / 2 - y
					   }
					   , stack
					} );
				}
			...
			if( possibles.length ) {
				let best
				     = { near: { x: self.stack.left + self.stack.image_width / 2 - x
				               , y: self.stack.top + self.stack.image_height / 2 - y
				        }
				       , stack: self.stack
				       };
	...
	select( stack, cards, cx, cy, mx, my ) {
		...
		stack.draw();
		this.draw();
		//console.log( "Initial select happened...", cx, cy, mx, my );
		this.frame.classList.add( "active" );
	}

	setDrag( x, y ) {
		//console.log( "Set drag (ofsx,y)", x, y );
		//this.ofsx = x;
		//this.ofsy = y;
	}
	...
	draw( noClear ) {
		...
		for( let card of this.cards ) {
			this.ctx.fillRect(
			     card.at.x / 100 * this.stack.image_width + this.cardx
			          + ( this.mx - this.ofsx )
			     , card.at.y / 100 * this.stack.image_height + this.cardy
			            + ( this.my - this.ofsy )
		...
		for( let c = this.cards.length - 1; c >= 0; c-- ) {
			...
			if( !card.flags.bFaceDown ) {
				//console.log( "draw card:", cx, cy, this.cardx, this.cardy, this.mx, this.my, this.ofsx, this.ofsy );
				this.ctx.drawImage(
				     cimg[ card.id ]
				...
				this.ctx.drawImage( cimg[ 52 ], card.at.x
				                  , card.at.y
				                  //, cx, cy
				                  , this.stack.image_width
				                  , this.stack.image_width * 1.5 );
		}
	}
	...
	animate( a ) {
		...
		for( let idx = 0; idx < this.turning.length; idx++ ) {
			...
			if( turn.end < ms ) {
				...
				if( cardDel > 0.5 ) {
					...
					// console.log( "Fliped, showing front now... ", show, cardDel,
					// turn.card.flags.bFaceDown?52:turn.card.id )
					this.ctx.drawImage(
					     cimg[ turn.card.flags.bFaceDown ? 52 : turn.card.id ]
					     , this.canvas.width * ( turn.card.thisStack.control.x )
					                 / 100
					            + turn.card.thisStack.control.image_width
					                 * turn.card.at.x
					                 / 100
					     , this.canvas.height * ( turn.card.thisStack.control.y )
					                 / 100
					            + turn.card.thisStack.control.image_height
					                 * turn.card.at.y
					                 / 100
					            + ( h / 2 ) * ( 1 - show )
					     , w, h * show );
					...
					// console.log( "Flip show: (showing back?)", show, cardDel,
					// turn.card.flags.bFaceDown?turn.card.id:52 );
					this.ctx.drawImage(
					     cimg[ turn.card.flags.bFaceDown ? turn.card.id : 52 ]
					     , this.canvas.width * ( turn.card.thisStack.control.x )
					                 / 100
					            + turn.card.thisStack.control.image_width
					                 * turn.card.at.x
		...
		for( let idx = 0; idx < this.moving.length; idx++ ) {
			...
			if( move.end < ms ) {
				...
				// console.log( "move expired... set card as down.", move.end,
				// move.card.name );
