...
let cardImages;
await fetch( "./images/cards/cardset2.jsox" )
     .then(
          async ( response ) => { return JSOX.parse( await response.text() ) } )
     .then( async ( data ) => {
	     if( data === "import" ) {
...
export const card_images = await Promise.all(
     cardImages.map( async ( card ) => {return getImage( card )} ) )
...
export const card_images_selected = card_images.map(
     ( img ) => processImage( img, Color( 255, 0, 0 ), Color( 0, 128, 0 )
                            , Color( 64, 64, 192 ) ) )
...
function processImage( image, r, g, b ) {
	...
	// var myImageData = converterCtx.getImageData(0, 0, image.width,
	// image.height);
	converterCtx.drawImage( image, 0, 0 );
	var myImageData
	     = converterCtx.getImageData( 0, 0, image.width, image.height );
...
export class card_stack_control {
	...
	flags        = { bVertical: false
	               ...
	               ,
	               bTurnTop: false
	               // click on pile turns over top card on stack, then plays it if
	               // possible.?
	               ,
	               bTurnToDiscard: false
	               // click on draw pile turns over top card on stack, then plays it
	               // on discard/table if possible.?
	               ,
	               bTurn3ToDiscard: false,
   };

	startup      = { nDrawAtStart: 0, nDrawDownAtStart: 0, nDrawAtDeal: 0, };

	active       = { nMustPlay: 0 // specific card must be played here...
	...
	//  step_x and step_y are the percentage of the control width/height that a
	//  card is offset. 2,2 is 1A + step_x*A
	card_width       = 1;
	card_height      = 1;
	...
	// psv_update_callback, // reference from registering an update callback for
	// this control - important for edit which may change which part of the deck
	// we watch....
	...
	constructor( options ) {
		...
		this.canvas.addEventListener( "touchstart"
		                            , ( evt ) => this.touch( "start", evt ) );
		this.canvas.addEventListener( "touchend"
		                            , ( evt ) => this.touch( "end", evt ) );
		this.canvas.addEventListener( "touchcancel"
		                            , ( evt ) => this.touch( "cancel", evt ) );
		this.canvas.addEventListener( "touchmove"
		                            , ( evt ) => this.touch( "move", evt ) );

		this.canvas.addEventListener( "mousedown"
		                            , ( evt ) => this.mouse( "down", evt ) );
		this.canvas.addEventListener( "mouseup"
		                            , ( evt ) => this.mouse( "up", evt ) );
		this.canvas.addEventListener( "mousemove"
		                            , ( evt ) => this.mouse( "move", evt ) );
		this.canvas.addEventListener( "mouseout"
		                            , ( evt ) => this.mouse( "out", evt ) );
		this.canvas.addEventListener( "mouseover"
		                            , ( evt ) => this.mouse( "over", evt ) );
		// this.canvas.addEventListener( "click", (evt)=>this.mouse( "click", evt
		// ) ); this.canvas.addEventListener( "dblclick", (evt)=>this.mouse(
		// "dblclick", evt ) );
		const resizeObserver = new ResizeObserver( entries => {
			for( let entry of entries ) {
				...
				for( let p = this.canvas.parentNode; p;
				...
				if( !this.flags.bVertical ) {
					this.card_width = width
					     - ( ( this.cards_wide - 1 ) * this.step_x ) / 100 * width;
					this.card_height = this.card_width * 1.5;
				} else {
					this.card_width  = width;
					this.card_height = height
					     - ( ( this.cards_high - 1 ) * this.step_y ) / 100 * height;
				}
	...
	set deck( val ) {
		...
		if( this.stack.control )
			console.log( "Card stack already has a control host?"
			           , this.stack.control );
	...
	touch( type, e ) {
		const into = e.target.getBoundingClientRect();
		switch( type ) {
		case "start":
			e.preventDefault();
			for( let touch of e.changedTouches ) {
				...
				if( old ) {
					old.x   = touch.clientX - into.left;
					old.y   = touch.clientY - into.top;
					old.new = true;
				} else {
					this.touches.push( { ID: touch.identifier
					                   , x: touch.clientX - into.left
					                   , y: touch.clientY - into.top
					                   , new: true
					} )
				}
			}
		...
		case "move": {
			...
			for( let touchChanged of e.changedTouches ) {
				var touch = this.touches.find( ( t ) => t.ID
				                               === touchChanged.identifier );
				if( touch ) {
					touch.x = touchChanged.clientX - into.left;
					touch.y = touchChanged.clientY - into.top;
				}
		...
		case "end": {
			e.preventDefault();
			for( let touchChanged of e.changedTouches ) {
				var touchIndex = this.touches.findIndex(
				     ( t ) => t.ID === touchChanged.identifier );
				if( touchIndex >= 0 ) {
			...
			if( this.active.nCardsSelected ) {
				card_drag_control.mouse( this.#dragControl
				                       , this.touchx + into.left
				                       , this.touchy + into.top, 0 );
				this.#dragControl._b = 0;
			}
		} break;
		}
		this.touchUpdate(into);
	}
	...
	touchUpdate(into) {
		if( this.touches.length == 1 ) {
			...
			if( t.new ) {
				...
				if( this.active.nCardsSelected ) {
					card_drag_control.mouse( this.#dragControl
					                       , this.touchx + this.canvas.offsetLeft
					                       , this.touchy + this.canvas.offsetTop, 1 );
				}
				// rotateStart.set( t.x, t.y );
				t.new = false;
			} else {
				this.#dragControl._b = this.b;
				this._b     = this.b;
				this.b      = 1;
				this.touchx = t.x;
				this.touchy = t.y;
				card_stack_control.mouse( this, this.touchx, this.touchy, this.b );
				if( this.active.nCardsSelected ) {
					//console.log( "touchxy and here left/top", this.touchx, this.touchy,  (this.canvas.offsetLeft)
			...
			if( this.active.nCardsSelected ) {
				card_drag_control.mouse( this.#dragControl
				                       , this.touchx +  this.canvas.offsetLeft
				                       , this.touchy +  this.canvas.offsetTop, 0 );
				this.#dragControl._b = 0;
			}
		}
	}
	...
	updateCards() {
		...
		for( let card = cards; card; card = card.next ) {
			...
			if( !card.next ) {
				...
				for( let c = card; c;
				   c       = ( c.me.ref != this.stack ) ? c.me.ref : null ) {
					cstack.push( c );
				}
				break;
			}
	...
	draw() {
		...
		for( let card of cstack ) {
			nsel--;
			if( card.flags.bFloating )
				continue;
			const cimg = ( this.game && this.game.selected_stack === this
			             && this.active.nCardsSelected > nsel )
			     ? card_images_selected
			     : card_images;

			if( card.flags.bFaceDown ) {
				card.at.x = x / this.canvas.width * 100;
				card.at.y = y / this.canvas.height * 100;
				this.ctx.drawImage( cimg[ card_images.length - 1 ], x, y
				...
				card.at.x = x / this.canvas.width * 100;
				card.at.y = y / this.canvas.height * 100;
				this.ctx.drawImage( cimg[ card.id ], x, y, this.card_width
				                  , this.card_height );
				y += ys;
				x += xs;
			}
			// console.log( "Simple draw stack:", this.deck_stack, card.name,
			// card.flags.bFaceDown?"down":"up" );
		}

		// if( ( this.canvas.width != stack.image_width ) ||(  this.canvas.height
		// != stack.image_height)  )
		...
		// ClearImageTo( surface, stack.background );
		{
			...
			if( !stack.flags.bStacked ) {
				...
				let col   = 0;
				let step_width
				     = ( ( ( stack.step_x < 0 ) ? -stack.step_x : stack.step_x )
				       * stack.width )
				     / 100;
				let step_height
				     = ( ( ( stack.step_y < 0 ) ? -stack.step_y : stack.step_y )
				       * stack.height )
				     / 100;
				let fd_step_width = ( ( ( stack.step_x < 0 ) ? -stack.fd_step_x
				                                             : stack.fd_step_x )
				                    * stack.width )
				     / 100;
				let fd_step_height = ( ( ( stack.step_y < 0 ) ? -stack.fd_step_y
				                                              : stack.fd_step_y )
				                     * stack.height )
				     / 100;
				let scaled_step_width  = ( stack.scaled_step_x < 0 )
				      ? -stack.scaled_step_x
				      : stack.scaled_step_x;
				let scaled_step_height = ( stack.scaled_step_y < 0 )
				     ? -stack.scaled_step_y
				     : stack.scaled_step_y;
				let x                  = ( this.canvas.width ) - stack.real_width;
				let y                  = ( this.canvas.height )
				     - stack.real_height; // surface.height - stack.height;

				let cardstack
				     = this.stack; // stack.deck.getStack( stack.deck_stack );
				/* should count the cards so we can better position this... */
				for( card = cardstack.last_card; card;
				   card   = ( card.me.field == "next" && card.me.ref ) ) {
					const card_image
					     = ( stack.active.nCardsSelected
					            ? card_images_selected
					            : card_images )[ card.flags.bFaceDown ? this.#max_id
					                                                  : card.id ];
					...
					if( card_image ) {
						...
						{
							if( !col && !row )
								this.ctx.drawImage( card_image, 0, 0, s_width, s_height
								...
								this.ctx.drawImage(
								     card_image, 0, 0, step_width, s_height
								     , x + col * stack.scaled_step_x
								     , y + row * stack.scaled_step_y
								     , stack.scaled_step_width, stack.real_height );
							} else if( !col && row )
								this.ctx.drawImage(
								     card_image, 0, 0, s_width, step_height
								     , x + col * stack.scaled_step_x
								     , y + row * stack.scaled_step_y, stack.real_width
								     , stack.scaled_step_height );
							else if( col && row )
								this.ctx.drawImage(
								     card_image, 0, 0, step_width, step_height
								     , x + col * stack.scaled_step_x
								     , y + row * stack.scaled_step_y, scaled_step_width
								     , stack.scaled_step_height );
						}
					}
				...
				let card = this.stack
				     .cards // stack.deck.getStack( stack.deck_stack ).cards;
				if( card ) {
					const card_image
					     = ( stack.active.nCardsSelected
					            ? card_images_selected
					...
					if( card_image ) {
						...
						if( s_height > card_image.height )
							s_height = card_image.height;
						this.ctx.drawImage( card_image, 0, 0, stack.width
						                  , stack.height, 0, 0, stack.width
						                  , stack.height );
					}
				}
			}
	...
	SetStackCards() {
		...
		stack.scaled_step_x = this.step_x * this.canvas.width
		     / ( this.card_width * this.step_x / 100 )
		     / 100
		stack.scaled_step_y = this.step_y * this.canvas.height
		     / ( ( this.card_height * this.step_y / 100 ) )
		     / 100
		stack.scaled_fd_step_x = this.card_width * this.fd_step_x / 100
		stack.scaled_fd_step_y = this.card_height * this.fd_step_y / 100
	}

	TransferCards( stack_to, n ) { const stack_from = this; }
			...
			stack.game.selected_stack = this;
			{
				...
				do {
					...
					if( stack.flags.bSelectAltSuit ) {
						if( _card ) {
							if( ( stack.#deck.CARD_SUIT( card.id ) % 2 )
							  != ( 1 - stack.#deck.CARD_SUIT( _card.id ) % 2 ) ) {
								count--;
								break;
							}
						}
					...
					if( stack.flags.bSelectSameSuit ) {
						if( _card ) {
							if( stack.#deck.CARD_SUIT( card.id )
							  != stack.#deck.CARD_SUIT( _card.id ) ) {
								count--;
								break;
							}
						}
					...
					if( stack.flags.bSelectPlusOne ) {
						if( _card ) {
							if( stack.#deck.CARD_NUMBER( card.id )
							  != ( 1 + stack.#deck.CARD_NUMBER( _card.id ) ) ) {
								count--;
								break;
							}
						}
					...
					if( stack.flags.bSelectMinusOne ) {
						if( _card ) {
							if( ( 1 + stack.#deck.CARD_NUMBER( card.id ) )
							  != ( stack.#deck.CARD_NUMBER( _card.id ) ) ) {
								count--;
								break;
							}
						}
	...
	static CanMoveCards( from, to, count ) {
		if( from.game == to.game ) {
			...
			if( !to_stack.cards ) {
				if( to.flags.bMustPlayWhenEmpty ) {
					let test = from.flags.bCompareTopOnly
					     ? from_stack.cards
					     : ( from_stack.getNthCard( from.active.nCardsSelected
					                              - 1 ) );
				...
				if( to.flags.bOnlyKingWhenEmpty ) {
					{
						// if( from.active.nCardsSelected > 1 )
						//	return false;
						// if( ( from_stack.cards.id % from.game.faces ) !=
						// (from.game.faces-1) ) 	return false;
						let n;
						for( n = 0; n < from.active.nCardsSelected; n++ ) {
			...
			if( to.flags.bSameSuit ) {
				...
				{
					if( ( ( to_stack.cards.id / from.#deck.faces ) | 0 )
					  != ( ( test.id / from.#deck.faces ) | 0 ) )
						return false;
				}
			}
			...
			if( to.flags.nMustPlay ) {
				if( from.#deck.CARD_NUMBER( from.stack.cards.id )
				  !== ( to.flags.nMustPlay - 1 ) )
					return false;
			}
					...
					if( test ) {
						if( to.flags.bOnlyPlusOne ) {
							if( to.flags.bLastKing )
								if( to.#deck.CARD_NUMBER( to_stack.cards.id )
								  == ( from.#deck.faces - 1 ) )
									return false;
	...
	static mouse( stack, x, y, b ) {
		// MyValidatedControlData( struct card_stack_control *, stack,
		// (PSI_CONTROL)pc );
		//  what can I do with a mouse?
		//  I can drraw on a frame... I don't need to be a control...
		// msg.textContent = `mouse : ${x} ${x/stack.canvas.width*100} ${y}
		// ${y/stack.canvas.height*100} ${b}`;
		if( !stack.game )
			return;
		//console.log( "Static stack mouse event:", stack.stack.name, x, y, b );
		if( ( b & 1 ) && !( stack._b & 1 ) ) {
			const card_stack = stack.stack;
			...
			if( card_stack.cards && card_stack.cards.flags.bFaceDown ) {
				...
				const scaled_fd_step_width
				     = stack.fd_step_x * stack.card_width / 100;
				const scaled_fd_step_height
				     = stack.fd_step_y * stack.card_height / 100;
				let fd_skip_y = 0;
				let fd_skip_x = 0;
				...
				if( y > ( ( count - 1 ) * scaled_step_height
				        + ( stack.canvas.width * 1.5 * 0.5 )
				        + fd_skip_y ) ) {
					count = 1;
				}
				...
				if( stack.game.selected_stack
				  && stack.game.selected_stack != stack ) {
					if( !stack.game.selected_stack.DoMoveCards( stack ) )
						stack.SelectCards(
						     count ); // pass the index of the card selected... may
						              // select up to this...
				} else
		...
		if( stack.#dragControl && stack.active.nCardsSelected ) {
			...
			for( let card = cards; card; card = card.next ) {
				...
				if( 0 )
					if( !card.next ) {
						for( let c = card; c;
						   c       = ( c.me.ref != stack.stack ) ? c.me.ref : null ) {
							if( !c.flags.bFaceDown ) {
								cstack.push( c );
							}
						}
			...
			// console.log( "button states are: " + b + " " + stack._b + " " +
			// stack.b );

			if( !( stack._b & 1 ) && ( stack.b & 1 ) ) {
				// console.log ( "selected to drag layer at " + x + "," + y );
				stack.#dragControl.select(
				     stack, cstack.slice( 0, stack.active.nCardsSelected ), cx, cy
				     , x + cx, y + cy );
			}
		}
		// stack._b = b;
